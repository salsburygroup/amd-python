...
inputs.add_argument('-top', action='store', dest='structure', help='Structure file corresponding to trajectory',
...
#  Make PCA projection
projection = PCA(n_components=0.95, svd_solver="full").fit_transform(data)
...
for i in range(0, num_components-1):
    progress = "\r Calculation on Component " + str(i) + " of " + str(num_components - 1)  # status
    sys.stdout.write(progress)
    sys.stdout.flush()  # report status to terminal output
    for j in range(i+1, num_components):
        temp = np.column_stack((projection[:, i], projection[:, j]))
        labels = MeanShift(n_jobs=-1, cluster_all=True).fit_predict(temp)
        num_clusters = max(labels) + 1  # 0-based index
        if num_clusters > 1:
            network.add_edge(i, j, weight=num_clusters)

nx.draw_spring(network, labels=label_dict)
adjacency_matrix = nx.convert_matrix.to_numpy_matrix(network)
np.savetxt(UserInput.out_name + '_PCA_adjacency_95.txt', adjacency_matrix, fmt='%1.0f')
plt.savefig(UserInput.out_name + '_PCA_network_95.png')
