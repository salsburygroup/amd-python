...
p2_tau=np.zeros((residues,bins,bins))
if tau == 0 :
    for j in range(residues):
        p2_tau[j]=np.histogramdd([delR[j], delR[j]], bins=bins)[0]/steps
else:
    for j in range(residues):
        p2_tau[j]=np.histogramdd([delR[j][:-tau], delR[j][tau:]], bins=bins)[0]/steps

# Calculating averaged ln of p1, p2, p2_tau, and p3
ln_jjtau=np.nansum(np.nansum(p2_tau*np.log(p2_tau), axis=1), axis=1)
