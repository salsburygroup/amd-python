...
class KMeans(object):
    ...
    def ik_means(self, data, k=None, theta=0, distance='SqEuclidean', p=None):
        ...
        while data.size > 0:
            ...
            while True:
                belongs_to_t_centroid = self.my_math.get_distance(data, t_centroid, distance,
                                                                  p) < self.my_math.get_distance(data, data_center,
                                                                                                 distance, p)
        ...
        if k is not None:
            if k < initial_centroids.shape[0]:
                initial_centroids = initial_centroids[n_entities_in_cluster.argsort()[
                                                      n_entities_in_cluster.size - k: n_entities_in_cluster.size], :]
        [u, centroids, ite, dist_tmp] = self.__k_means_batch(original_data, initial_centroids.shape[0],
                                                             initial_centroids, distance, p)
