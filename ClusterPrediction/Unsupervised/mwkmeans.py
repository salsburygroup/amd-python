...
class MWKMeans(object):
    ...
    def mwk_means(self, data, k, p, init_centroids=None, init_weights=None, replicates=1, max_ite=100,
                  gradient=np.array([0.001]), cutoff=0.0001):
        ...
        return wkm.wk_means(data, k, p, init_centroids, init_weights, 'Minkowski_pthPower', replicates, p, max_ite,
                            gradient=gradient, cutoff=cutoff)

    def imwk_means(self, data, p, k, theta, gradient, cutoff):
        ...
        original_data = data[:]
        data_center = self.my_math.get_center(data, 'Minkowski_pthPower', p, gradient, cutoff)
        ...
        while data.size > 0:
            ...
            while True:
                belongs_to_t_centroid = self.my_math.get_distance(data, t_centroid, 'Minkowski_pthPower', p,
                                                                  t_weights ** p) < self.my_math.get_distance(data,
                                                                                                              data_center,
                                                                                                              'Minkowski_pthPower',
                                                                                                              p,
                                                                                                              t_weights ** p)
                ...
                nt_centroid = self.my_math.get_center(data[belongs_to_t_centroid, :], 'Minkowski_pthPower', p, gradient,
                                                      cutoff)
                if np.array_equal(t_centroid, nt_centroid) or np.array_equal(belongs_to_t_centroid,
                                                                             previous_belongs_to_t_centroid):
                    break
                t_centroid = nt_centroid[:]
                previous_belongs_to_t_centroid = belongs_to_t_centroid[:]
                # update weights
                t_weights = wkm._get_dispersion_based_weights(data, np.vstack([data_center, t_centroid]), 2, p,
                                                              belongs_to_t_centroid.astype(int), n_features,
                                                              'Minkowski_pthPower', p, 0.01)[1, :]
