...
class MyMath(object):
    ...
    def get_center(self, x, distance, p, gradient, cutoff):
        print(gradient)
        print(cutoff)
        if distance is None or distance == 'SqEuclidean':
            ...
            while True:
                ...
                x_center[distance_to_nx_center < distance_to_x_center] = nx_center[
                    distance_to_nx_center < distance_to_x_center]
                distance_to_x_center[distance_to_nx_center < distance_to_x_center] = distance_to_nx_center[
                    distance_to_nx_center < distance_to_x_center]
                if np.all(np.abs(gradient) < cutoff):
                    return x_center
        else:
            raise Exception('Unrecognised distance')
