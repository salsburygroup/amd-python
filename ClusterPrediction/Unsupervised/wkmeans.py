...
class WKMeans(object):
    ...
    def _get_dispersion_based_weights(self, data, centroids, k, beta, u, n_features, distance, p,
                                      dispersion_update='mean'):
        ...
        if beta != 1:
            ...
            for k_i in range(k):
                for f_i in range(n_features):
                    weights[k_i, f_i] = 1 / (
                    (dispersion[k_i, f_i].repeat(n_features) / dispersion[k_i, :]) ** exp).sum()
        else:
            for k_i in range(k):
    ...
    def __wk_means(self, data, k, beta, centroids=None, weights=None, max_ite=100, distance='SqEuclidean', p=None,
        ...
        while ite <= max_ite:
            ...
            for k_i in range(k):
                ...
                centroids[k_i, :] = self.my_math.get_center(data[entities_in_k, :], distance, p, gradient, cutoff)
            # update weights
    ...
    def wk_means(self, data, k, beta, init_centroids=None, init_weights=None, distance='SqEuclidean', replicates=1,
                 p=None, max_ite=100, gradient=np.array([0.001]), cutoff=0.0001):
        # Weighted K-Means
        ...
        for replication_i in range(replicates):
            ...
            for i in range(max_ite):
                [u, centroids, weights, ite, dist_tmp] = self.__wk_means(data, k, beta, init_centroids, init_weights,
                                                                         max_ite, distance, p, gradient, cutoff)
                if u[0] != -1:
                    break
            if u[0] == -1:
